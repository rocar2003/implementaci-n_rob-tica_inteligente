...
class MasterControlNode(Node):
    ...
    def timer_callback(self):
        if self.is_stopping:
            ...
            if time() - self.turn_left_start_time < 1.7:
                self.robot_vel.linear.x = 0.2
                self.robot_vel.angular.z = 0.0
                self.velocity_publisher.publish(self.robot_vel)
            elif time() - self.turn_left_start_time < 6.5 and time() - self.turn_left_start_time > 1.7:
                self.robot_vel.linear.x = 0.0
                self.robot_vel.angular.z = 0.4
                self.velocity_publisher.publish(self.robot_vel)
            elif time() - self.turn_left_start_time < 7.3 and time() - self.turn_left_start_time > 6.5:
            ...
            if time() - self.right_start_time < 1.2:
                self.robot_vel.linear.x = 0.2
                self.robot_vel.angular.z = 0.0
                self.velocity_publisher.publish(self.robot_vel)
            elif time() - self.right_start_time < 9.2 and time() - self.right_start_time > 1.2:
                self.robot_vel.linear.x = 0.0
                self.robot_vel.angular.z = -0.2
                self.velocity_publisher.publish(self.robot_vel)
            elif time() - self.right_start_time < 10.4 and time() - self.right_start_time > 9.2:
                self.robot_vel.linear.x = 0.1
                self.robot_vel.angular.z = 0.0
                self.velocity_publisher.publish(self.robot_vel)
            else:
